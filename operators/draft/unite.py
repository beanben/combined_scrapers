# project specific
from operators.utilities import get_country, combine_items, get_tenancy_type, get_room_type, get_currency, pw_to_pm, weeks_to_months
...
class UniteSpider(Spider):
    ...
    def parse_cities(self, res):
        # links ignored
        links_ignored = ["/about-us", "/help", "/group-booking", "/login",
                         "/the-common-room", "/secure", "/account", "/products"]

        # loop through cities
        for link in res.css('div[id="Navigation"]').css('a[href]'):
            ...
            pdb.set_trace()
            if not any(el in city_url.lower() for el in links_ignored) and len(city_url) > 1:
    ...
    def parse_properties(self, res):
        ...
        start = script.find("CityMapFilterListing, ") + \
            ...
            '), document.getElementById("react_')
